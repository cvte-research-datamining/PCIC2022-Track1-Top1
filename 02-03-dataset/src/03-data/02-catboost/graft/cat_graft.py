...
if __name__ == '__main__':
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(train, y_train)):
        ...
        model.save_model(f'cat_phase1_{fold_}')
        print('train done!')
        # model.load_model(f'cat_phase1_{fold_}')
        ...
        sub_test1['prob'] += model.predict_proba(test.drop(['sequence'], axis=1))[:, 1] / folds.n_splits
    ...
    test = test.merge(test_prob[['sequence', 'prob']], on='sequence', how='left')
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(train, y_train)):
        ...
        model.save_model(f'cat_phase2_{fold_}')
        print('train done!')
        ...
        sub_test2['prob'] += model.predict_proba(test.drop(['sequence'], axis=1))[:, 1] / folds.n_splits
